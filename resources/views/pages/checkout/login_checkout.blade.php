        ...
        <form role="form" action="{{URL::to('/add-customer')}}" method="POST">
          {{ csrf_field() }}
           <fieldset>							
             ...
             <div class="form-group">
               <input type="email" name="khEmaildk" class="form-control input-lg @error('khEmaildk') is-invalid @enderror" placeholder="Email">
               @error('khEmaildk')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
             </div>
             <div class="form-group">
               <input type="password" name="khMatKhaudk" class="form-control input-lg @error('khMatKhaudk') is-invalid @enderror" placeholder="Mật khẩu">
               @error('khMatKhaudk')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
             </div>
