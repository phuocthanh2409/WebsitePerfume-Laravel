...
class LoaiSanPham extends Controller
{
    ...
    public function show_loaisanpham_home($loaiMa)
    {
        ...
        $loaisanpham_by_id = DB::table('sanpham')
            ...
            ->get();
        $loaisanpham_name = DB::table('loaisanpham')->where('thuonghieu.thMa', $thMa)->limit(1)->get();
        return view('pages.loaisanpham.show_loaisanpham')->with('loaisanpham', $loaisanpham)->with('thuonghieu', $thuonghieu)->with('muasanpham', $muasanpham)->with('loaisanpham_by_id', $loaisanpham_by_id);
    }
}
