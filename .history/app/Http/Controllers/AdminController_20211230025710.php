...
class AdminController extends Controller
{
    ...
    public function save_adminthongtin(Request $request)
    {
        ...
        if ($maquantri) {
            ...
            return Redirect::to('admin')->with('error', 'Vui lòng đăng nhập để thực hiện chức năng');
        }
    }
    ...
    public function save_matkhauquantri(Request $request)
    {
        ...
        if ($maquantri) {
            $data = $request->all();
            $matkhaucu = md5($data['quantrimatkhaucu']);
            $thongtin = QuanTriVien::where('qtMa', '=', $maquantri)->where('qtMatKhau', '=', $matkhaucu)->first();
            if ($thongtin) {
                $thongtin->qtMatKhau = md5($data['quantrimatkhaumoi']);
                $thongtin->save();
                return Redirect::to('doimatkhau-quantri')->with('success', 'Cập nhật thông tin tài khoản thành công');
            } else {
                return Redirect::to('doimatkhau-quantri')->with('error', 'Mật khẩu hiện tại không đúng. Vui lòng nhập lại');
            }
        } else {
