...
class CheckoutController extends Controller
{
    ...
    public function add_customer(Request $request)
    {
        ...
        if (KhachHang::where('khEmail', '=', $request->khEmail)->exists()) {
            return Redirect::to('/login-checkout')->with('error', 'Email này đã được đăng kí. Vui lòng vào email để xác nhận tài khoản');
        } else {
            $data = array();
    ...
    public function create_account($email, $token)
    {
        ...
        if ($count > 0) {
            ...
            return redirect('quen-mat-khau')->with('error', 'Vui lòng chọn gửi lại mail kích hoạt vì link đã quá hạn');
        }
    }
