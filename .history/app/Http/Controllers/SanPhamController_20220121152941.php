...
class SanPhamController extends Controller
{
    ...
    public function save_sanpham(Request $request)
    {
        ...
        if ($data['spGiaGoc'] > $data['spGia']) {
            return Redirect::to('all-sanpham')->with('error', 'Thêm sản phẩm thất bại. Giá gốc không được lớn hơn giá bán');
        } else {
    ...
    {
        $this->AuthLogin();
        $check = ChiTietDonHang::where('spMa', $spMa)->first();
        if ($check == null) {
            DB::table('sanpham')->where('spMa', $spMa)->delete();
            return Redirect::to('all-sanpham')->with('success', 'Xóa sản phẩm thành công');
        } else {
            return Redirect::to('all-sanpham')->with('success', 'Xóa sản phẩm thất bại. Sản phẩm đã được đặt hàng không thể xóa');
        }
    }
