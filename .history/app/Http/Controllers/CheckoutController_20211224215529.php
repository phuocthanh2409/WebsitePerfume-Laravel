...
class CheckoutController extends Controller
{
    ...
    public function add_customer(Request $request)
    {
        ...
        if (KhachHang::where('khEmail', '=', $request->khEmail)->exists()) {
            return Redirect::to('/login-checkout')->with('error', 'Email này đã được đăng kí');
        } else {
            $data = array();
            ...
            $link_reset_pass = url('/create-account?email=' . $to_email . '&token=' . $token_random);

            $data = array("name" => $title_mail, "body" => $link_reset_pass, 'email' => $data['khEmail']);

            Mail::send('pages.checkout.create_acount', ['data' => $data], function ($message) use ($title_mail, $data) {
            ...
            return Redirect::to('/show-cart')->with('success', 'Vui lòng truy cập email để kích hoạt tài khoản');
        }
    }
    ...
    public function login_customer(Request $request)
    {
        ...
        if ($result) {
            ...
            return Redirect::to('/login-checkout')->with('error', 'Tài khoản chưa được kích hoạt. Vui lòng vào email để kích hoạt tài khoản');
        } else {
