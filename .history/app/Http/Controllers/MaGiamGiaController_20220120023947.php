...
class MaGiamGiaController extends Controller
{
    ...
    public function insert_magiamgia_code(Request $request)
    {
        ...
        $data = $request->all();
        if ($data['magiamgia_PhuongThuc'] == 0) {
            if ($data['magiamgia_GiaTri'] > 90) {
                return Redirect::to('list-magiamgia')->with('error', 'Thêm mã giảm giá thất bại. Giá trị mã giảm giá phải nhỏ hơn 90 %');
            } else {
                ...
                $magiamgia->save();
                return Redirect::to('list-magiamgia')->with('success', ' mã giảm giá thành công');
            }
        } else {
            if ($data['magiamgia_GiaTri'] < 1000) {
                return Redirect::to('list-magiamgia')->with('error', 'Thêm mã giảm giá thất bại. Giá trị mã giảm giá phải lớn hơn 1000');
            } else {
                $magiamgia = MaGiamGia::find($mggMa);
                $magiamgia->mggTen = $data['magiamgia_Ten'];
                $magiamgia->mggCode = $data['mggCode'];
    ...
    public function update_magiamgia(Request $request, $mggMa)
    {
        ...
        if ($data['magiamgia_PhuongThuc'] == 0) {
            if ($data['magiamgia_GiaTri'] > 90) {
                return Redirect::to('list-magiamgia')->with('error', 'Cập nhật mã giảm giá thất bại. Giá trị mã giảm giá phải nhỏ hơn 90 %');
            } else {
                $magiamgia = MaGiamGia::find($mggMa);
                $magiamgia->mggTen = $data['magiamgia_Ten'];
                $magiamgia->mggCode = $data['mggCode'];
            ...
            if ($data['magiamgia_GiaTri'] < 1000) {
                return Redirect::to('list-magiamgia')->with('error', 'Cập nhật mã giảm giá thất bại. Giá trị mã giảm giá phải lớn hơn 1000');
            } else {
                $magiamgia = MaGiamGia::find($mggMa);
                $magiamgia->mggTen = $data['magiamgia_Ten'];
                $magiamgia->mggCode = $data['mggCode'];
