...
class NuocHoaController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if ($keywords == '') {
            return view('pages.sanpham.search')->with('loaisanpham', $loaisanpham)->with('thuonghieu', $thuonghieu)->with('muasanpham', $muasanpham);
        } else {
            $search_product = DB::table('sanpham')->where('spTrangThai', '1')->where('spTen', 'like', '%' . $keywords . '%')->paginate(9);
