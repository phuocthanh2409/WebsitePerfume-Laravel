...
class CheckoutController extends Controller
{
    ...
    public function confirm_order(Request $request)
    {
        ...
        $thuonghieu = DB::table('thuonghieu')->where('thTrangThai', '1')->orderby('thMa', 'asc')->get();
        $muasanpham = DB::table('muasanpham')->where('muaTrangThai', '1')->orderby('muaMa', 'asc')->get();
        return view('pages.history.history')->with('loaisanpham', $loaisanpham)->with('thuonghieu', $thuonghieu)->with('muasanpham', $muasanpham)->with('success', 'Bạn đã đặt hàng thành công');
    }
}
