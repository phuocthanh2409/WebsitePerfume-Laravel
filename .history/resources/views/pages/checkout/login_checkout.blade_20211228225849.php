        ...
        <form role="form" action="{{URL::to('/add-customer')}}" method="POST">
          {{ csrf_field() }}
           <fieldset>							
             ...
             <div class="form-group">
              <input type="text" name="khTen" class="form-control input-lg @error('khTen') is-invalid @enderror" placeholder="Họ và tên" required>
              @error('khTen')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
            </div>
             <div class="form-group">
               <input type="email" name="khEmail" class="form-control input-lg @error('khEmail') is-invalid @enderror" placeholder="Email" required>
               @error('khEmail')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
             </div>
             <div class="form-group">
               <input type="password" name="khMatKhau" class="form-control input-lg @error('khTen') is-invalid @enderror" placeholder="Mật khẩu" required>
               @error('khTen')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
             </div>
             <div class="form-group">
              <input type="number" name="khSoDienThoai" class="form-control input-lg @error('khTen') is-invalid @enderror" placeholder="Số điện thoại" required>
              @error('khTen')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
            </div>
