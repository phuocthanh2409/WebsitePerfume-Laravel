          ...
          <div class="col-lg-12">
              <div class="breadcrumb__text">
                  ...
                  <div class="breadcrumb__links">
                      ...
                      @foreach($loaisanpham_name as $key => $loainame)
                      <span>{{$loainame->loaiTen}}</span>
                      @endforeach
                  </div>
              </div>
          </div>
  ...
  <div class="container">
      <div class="row">
          ...
          <div class="col-lg-9">
              ...
              <div class="row">
                @foreach($loaisanpham_by_id as $key => $loai)
                  <div class="col-lg-4 col-md-6 col-sm-6">
                      <div class="product__item">
                        <a href="{{URL::to('/chi-tiet-san-pham/'.$loai->spMa)}}">
                          <div class="product__item__pic set-bg" data-setbg="{{URL::to('public/uploads/product/'.$loai->spHinhAnh)}}">
                          </div>
                        </a>
                          <div class="product__item__text">
                            <form>
                                {{csrf_field()}} 
                                <input type="hidden" value="{{$loai->spMa}}" class="cart_product_id_{{$loai->spMa}}">
                                <input type="hidden" value="{{$loai->spTen}}" class="cart_product_name_{{$loai->spMa}}">
                                <input type="hidden" value="{{$loai->spHinhAnh}}" class="cart_product_image_{{$loai->spMa}}">
                                <input type="hidden" value="{{$loai->spSoLuong}}" class="cart_product_quantity_{{$loai->spMa}}">
                                <input type="hidden" value="{{$loai->spGia}}" class="cart_product_price_{{$loai->spMa}}">
                                <input type="hidden" value="1" class="cart_product_qty_{{$loai->spMa}}">
                                <div class="product__item__text">
                                  <h6>{{$loai->spTen}}</h6>
                                  <button style="padding: 0;
                                  border: none;
                                  background: none;" type="button" class="add-to-cart" data-id_product="{{$loai->spMa}}" name="add-to-cart">
                                  <a class="add-cart">
                                      + Thêm vào giỏ hàng
                                    </a>
                                  </button>
                                  <h5>{{number_format($loai->spGia).' VNĐ'}}</h5>
                                </div>
                              </form>
                          </div>
                      </div>
