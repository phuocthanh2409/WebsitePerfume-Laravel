                      ...
                      <div class="product__item">
                        <a href="{{URL::to('/chi-tiet-san-pham/'.$gia1->spMa)}}">
                          ...
                          <div class="product__item__text">
                            <form>
                                {{csrf_field()}} 
                                <input type="hidden" value="{{$gia1->spMa}}" class="cart_product_id_{{$gia1->spMa}}">
                                <input type="hidden" value="{{$gia1->spTen}}" class="cart_product_name_{{$gia1->spMa}}">
                                <input type="hidden" value="{{$gia1->spHinhAnh}}" class="cart_product_image_{{$gia1->spMa}}">
                                <input type="hidden" value="{{$gia1->spSoLuong}}" class="cart_product_quantity_{{$gia1->spMa}}">
                                <input type="hidden" value="{{$gia1->spGia}}" class="cart_product_price_{{$gia1->spMa}}">
                                <input type="hidden" value="1" class="cart_product_qty_{{$gia1->spMa}}">
                                <div class="product__item__text">
                                  <h6>{{$gia1->spTen}}</h6>
                                  <button style="padding: 0;
                                  border: none;
                                  background: none;" type="button" class="add-to-cart" data-id_product="{{$gia1->spMa}}" name="add-to-cart">
                                  <a class="add-cart">
                                      + Thêm vào giỏ hàng
                                    </a>
                                  </button>
