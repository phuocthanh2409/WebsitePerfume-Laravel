          ...
          <div class="col-lg-12">
              <div class="breadcrumb__text">
                  ...
                  <div class="breadcrumb__links">
                      ...
                      <span>Sản phẩm từ 2.000.000 VNĐ đến 5.000.000 VNĐ</span>
                  </div>
              </div>
          </div>
      </div>
  ...
  <div class="container">
      <div class="row">
          ...
          <div class="col-lg-9">
              ...
              <div class="row">
                @foreach($gia_by_id_3 as $key => $gia3)
                  <div class="col-lg-4 col-md-6 col-sm-6">
                      <div class="product__item">
                        <a href="{{URL::to('/chi-tiet-san-pham/'.$gia3->spMa)}}">
                          <div class="product__item__pic set-bg" data-setbg="{{URL::to('public/uploads/product/'.$gia3->spHinhAnh)}}">
                          </div>
                        </a>
                          <div class="product__item__text">
                            <form>
                                {{csrf_field()}} 
                                <input type="hidden" value="{{$gia3->spMa}}" class="cart_product_id_{{$gia3->spMa}}">
                                <input type="hidden" value="{{$gia3->spTen}}" class="cart_product_name_{{$gia3->spMa}}">
                                <input type="hidden" value="{{$gia3->spHinhAnh}}" class="cart_product_image_{{$gia3->spMa}}">
                                <input type="hidden" value="{{$gia3->spSoLuong}}" class="cart_product_quantity_{{$gia3->spMa}}">
                                <input type="hidden" value="{{$gia3->spGia}}" class="cart_product_price_{{$gia3->spMa}}">
                                <input type="hidden" value="1" class="cart_product_qty_{{$gia3->spMa}}">
                                <div class="product__item__text">
                                  <h6>{{$gia3->spTen}}</h6>
                                  <button style="padding: 0;
                                  border: none;
                                  background: none;" type="button" class="add-to-cart" data-id_product="{{$gia3->spMa}}" name="add-to-cart">
                                  <a class="add-cart">
                                      + Thêm vào giỏ hàng
                                    </a>
                                  </button>
                                  <h5>{{number_format($gia3->spGia).' VNĐ'}}</h5>
                                </div>
                              </form>
                          </div>
                      </div>
