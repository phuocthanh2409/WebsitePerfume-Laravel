          ...
          <div class="col-lg-12">
              <div class="breadcrumb__text">
                  ...
                  <div class="breadcrumb__links">
                      ...
                      <span>Sản phẩm trên 10.000.000 VNĐ</span>
                  </div>
              </div>
          </div>
      </div>
  ...
  <div class="container">
      <div class="row">
          ...
          <div class="col-lg-9">
              ...
              <div class="row">
                @foreach($gia_by_id_5 as $key => $gia5)
                  <div class="col-lg-4 col-md-6 col-sm-6">
                      <div class="product__item">
                        <a href="{{URL::to('/chi-tiet-san-pham/'.$gia5->spMa)}}">
                          <div class="product__item__pic set-bg" data-setbg="{{URL::to('public/uploads/product/'.$gia5->spHinhAnh)}}">
                          </div>
                        </a>
                          <div class="product__item__text">
                            <form>
                                {{csrf_field()}} 
                                <input type="hidden" value="{{$gia5->spMa}}" class="cart_product_id_{{$gia5->spMa}}">
                                <input type="hidden" value="{{$gia5->spTen}}" class="cart_product_name_{{$gia5->spMa}}">
                                <input type="hidden" value="{{$gia5->spHinhAnh}}" class="cart_product_image_{{$gia5->spMa}}">
                                <input type="hidden" value="{{$gia5->spSoLuong}}" class="cart_product_quantity_{{$gia5->spMa}}">
                                <input type="hidden" value="{{$gia5->spGia}}" class="cart_product_price_{{$gia5->spMa}}">
                                <input type="hidden" value="1" class="cart_product_qty_{{$gia5->spMa}}">
                                <div class="product__item__text">
                                  <h6>{{$gia5->spTen}}</h6>
                                  <button style="padding: 0;
                                  border: none;
                                  background: none;" type="button" class="add-to-cart" data-id_product="{{$gia5->spMa}}" name="add-to-cart">
                                  <a class="add-cart">
                                      + Thêm vào giỏ hàng
                                    </a>
                                  </button>
                                  <h5>{{number_format($gia5->spGia).' VNĐ'}}</h5>
                                </div>
                              </form>
                          </div>
                      </div>
