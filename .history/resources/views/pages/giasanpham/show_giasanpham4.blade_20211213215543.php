          ...
          <div class="col-lg-12">
              <div class="breadcrumb__text">
                  ...
                  <div class="breadcrumb__links">
                      ...
                      <span>Sản phẩm từ 5.000.000 VNĐ đến 10.000.000 VNĐ</span>
                  </div>
              </div>
          </div>
      </div>
  ...
  <div class="container">
      <div class="row">
          ...
          <div class="col-lg-9">
              ...
              <div class="row">
                @foreach($gia_by_id_4 as $key => $gia4)
                  <div class="col-lg-4 col-md-6 col-sm-6">
                      <div class="product__item">
                        <a href="{{URL::to('/chi-tiet-san-pham/'.$gia4->spMa)}}">
                          <div class="product__item__pic set-bg" data-setbg="{{URL::to('public/uploads/product/'.$gia4->spHinhAnh)}}">
                          </div>
                        </a>
                          <div class="product__item__text">
                            <form>
                                {{csrf_field()}} 
                                <input type="hidden" value="{{$gia4->spMa}}" class="cart_product_id_{{$gia4->spMa}}">
                                <input type="hidden" value="{{$gia4->spTen}}" class="cart_product_name_{{$gia4->spMa}}">
                                <input type="hidden" value="{{$gia4->spHinhAnh}}" class="cart_product_image_{{$gia4->spMa}}">
                                <input type="hidden" value="{{$gia4->spSoLuong}}" class="cart_product_quantity_{{$gia4->spMa}}">
                                <input type="hidden" value="{{$sgia4p->spGia}}" class="cart_product_price_{{$gia4->spMa}}">
                                <input type="hidden" value="1" class="cart_product_qty_{{$gia4->spMa}}">
                                <div class="product__item__text">
                                  <h6>{{$gia4->spTen}}</h6>
                                  <button style="padding: 0;
                                  border: none;
                                  background: none;" type="button" class="add-to-cart" data-id_product="{{$gia4->spMa}}" name="add-to-cart">
                                  <a class="add-cart">
                                      + Thêm vào giỏ hàng
                                    </a>
                                  </button>
                                  <h5>{{number_format($gia4->spGia).' VNĐ'}}</h5>
                                </div>
                              </form>
                          </div>
                      </div>
