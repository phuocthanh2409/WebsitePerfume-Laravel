      ...
      <div class="card-body">
        ...
        <form action="{{URL::to('/update-info/'.$val->ttMa)}}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group row">
            ...
            <div class="col-12 col-sm-8 col-lg-6">
              <textarea rows="10" class="form-control @error('info_map') is-invalid @enderror" placeholder="Link bản đồ Google" name="info_map" required="">{{$val->ttMap}}</textarea>
              @error('info_map')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="form-group row">
            ...
            <div class="col-12 col-sm-8 col-lg-6">
              <textarea rows="10" class="form-control @error('info_fanpage') is-invalid @enderror" placeholder="Link fanpage Facebook" name="info_fanpage" required="">{{$val->ttFanpage}}</textarea>
              @error('info_fanpage')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="form-group row">
            ...
            <div class="col-12 col-sm-6">
              <input name="info_image" class="inputfile @error('info_contact') is-invalid @enderror" id="file-1" type="file" name="file-1" data-multiple-caption="{count} files selected" multiple=""  required="">
              @error('info_contact')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
