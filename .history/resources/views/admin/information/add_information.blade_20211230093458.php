      ...
      <div class="card-body">
        ...
        <form action="{{URL::to('/update-info/'.$val->ttMa)}}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group row">
            ...
            <div class="col-12 col-sm-8 col-lg-6">
              <textarea rows="10" class="form-control @error('info_contact') is-invalid @enderror" placeholder="Link fanpage Facebook" name="info_fanpage" required="">{{$val->ttFanpage}}</textarea>
              @error('info_contact')
                    <div class="alert alert-danger">{{ $message }}</div>
              @enderror
            </div>
