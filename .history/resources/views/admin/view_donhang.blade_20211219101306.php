        ...
        <table class="table table-striped table-hover table-fw-widget" id="table5">
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              ...
              <td>
                <input type="number" {{$order_status != 1 ? 'disabled' : ''}} class="order_qty_{{$details->spMa}}" min="1" value="{{$details->spSoLuongMua}}" name="product_sales_quantity">
            ...
            <tr>
              <td colspan="9" style="text-align: right">
                ...
                <?php
                  ...
                  echo '<h6 style="font-weight: bold" >Giảm giá: '.number_format($total_after_coupon,0,',','.').' VNĐ'.'</br></h6>';
                  $total_coupon = $total - $total_after_coupon + $details->spPhiVanChuyen;
                ?>
                @else 
                  <?php
                  echo '<h6 style="font-weight: bold" >Giảm giá: '.number_format($coupon_number,0,',','.').' VNĐ'.'</br></h6>';
                  $total_coupon = $total - $coupon_number + $details->spPhiVanChuyen; 
                  ?>
                @endif
                <h6 style="font-weight: bold">Phí ship: {{number_format($details->spPhiVanChuyen,0,',','.')}} VNĐ <br></h6>
                <h6 style="font-weight: bold">Thanh toán: {{number_format($total_coupon,0,',','.')}} VNĐ</h6>
              </td>
            </tr>
            <tr>
              <td colspan="9">
                @foreach($order as $key => $or)
                  ...
                  <form>
                    @csrf
                    <select class="form-control order_details_huy">
                      {{-- <option id="{{$or->dhMa}}" disabled value="1">Chưa xử lý</option> --}}
                      {{-- <option id="{{$or->dhMa}}" disabled value="2">Đã xử lý - Đã giao hàng</option> --}}
                      <option id="{{$or->dhMa}}" value="3" selected>Hủy đơn hàng</option>
                    </select>
                  </form>
                  @endif
                @endforeach
