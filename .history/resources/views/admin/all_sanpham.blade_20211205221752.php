        ...
        <table class="table table-striped table-hover table-fw-widget" id="table5">
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              <td>{{$sp_pro->spTen}}</td>
              <td>{{$sp_pro->spGia}}</td>
              <td>{{$sp_pro->spGiaKhuyenMai}}</td>
              <td>{{$sp_pro->spDungTich}}</td>
              <td>{{$sp_pro->spSoLuong}}</td>
              <td>{{$sp_pro->spHinhAnh}}</td>
              <td>{{$sp_pro->spHinhThuc}}</td>
              ...
              <td class="actions"><a class="icon" onclick="return confirm('Bạn có chắc là muốn xóa ?')" href="{{URL::to('/delete-sanpham/'.$brand_pro->thMa)}}"><i class="mdi mdi-delete"></i></a></td>
            </tr>
            @endforeach
          </tbody>
        </table>
