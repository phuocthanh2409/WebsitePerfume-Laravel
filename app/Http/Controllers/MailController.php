...
class MailController extends Controller
{
    ...
    public function reset_new_pass(Request $request)
    {
        $request->validate(
            [
                'password_account' => 'required|max:191'
            ],
            [
                'password_account.required' => 'Vui lòng nhập mật khẩu mới để thực hiện chức năng',
                'password_account.max' => 'Mật khẩu của bạn quá dài. Vui lòng kiểm tra lại',
            ]
        );
        $data = $request->all();
        $token_random = Str::random();
